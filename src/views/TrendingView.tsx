...
export const TrendingView: React.FC<TrendingViewProps> = ({
  ...
  const fetchTopicVideos = async (topicId: string, forceFresh = false) => {
    ...
    try {
      ...
      const endpoint = topicId === 'all' 
        ? '/api/trending?category=all' 
        : `/api/trending?category=${topicId.toLowerCase()}`;

      const res = await fetch(endpoint);
      if (res.ok) {
        const data = await res.json();
        if (Array.isArray(data.videos) && data.videos.length > 0) {
          const allowed = data.videos.filter((v: Video) => 
            v && !recommendationEngine.isVideoHidden(v.id) && !recommendationEngine.isChannelBlocked(v.channelId, v.channelTitle)
          );
          setTopicVideos(prev => ({
            ...prev,
            [topicId]: allowed,
