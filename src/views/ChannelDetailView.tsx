...
export const ChannelDetailView: React.FC<ChannelDetailViewProps> = ({
  ...
  const [subscriberCount, setSubscriberCount] = useState<string>('Subscribers');
  const [bannerUrl, setBannerUrl] = useState<string>('https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=1400&auto=format&fit=crop&q=80');
  const [channelDesc, setChannelDesc] = useState<string>('');
  const [realAvatar, setRealAvatar] = useState<string>(channelAvatar || '');
  ...
  const loadDefaultVideos = async () => {
    ...
    try {
      ...
      if (data.channel) {
        if (data.channel.subscriberCount) setSubscriberCount(data.channel.subscriberCount);
        if (data.channel.banner) setBannerUrl(data.channel.banner);
        if (data.channel.avatar) setRealAvatar(data.channel.avatar);
        if (data.channel.description) setChannelDesc(data.channel.description);
      }
        ...
        <div className="flex items-start sm:items-center gap-4">
          ...
          <div className="min-w-0 space-y-1">
            ...
            <p className={`text-xs sm:text-sm font-medium ${isLight ? 'text-slate-600' : 'text-slate-400'}`}>
              @{channelTitle.toLowerCase().replace(/[^a-z0-9]/g, '')} • {subscriberCount} • {channelVideos.length} videos
            </p>
            <p className={`text-xs line-clamp-1 max-w-xl ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>
              {channelDesc || `Official uploads and streams for ${channelTitle}, verified privacy-friendly playback via FreeTube.`}
            </p>
          </div>
        </div>
