...
import { 
  ...
  Smartphone, Download, Film, FastForward
} from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const [recentSearches, setRecentSearches] = useState<string[]>(() => {
    try {
      const saved = localStorage.getItem('freetube_search_history');
      if (saved) return JSON.parse(saved);
    } catch {}
    return ['Trending Today', 'Veritasium', 'Marques Brownlee'];
  });
  ...
  const handleSubmit = (query: string) => {
    ...
    setRecentSearches(prev => {
      ...
      try {
        localStorage.setItem('freetube_search_history', JSON.stringify(updated));
      } catch {}
      return updated;
    });
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="absolute right-2 flex items-center gap-1">
            ...
            {onOpenSearchFilters && (
              <button
                type="button"
                id="search-filter-modal-btn"
                onClick={onOpenSearchFilters}
                className={`relative p-1.5 rounded-xl transition-all active:scale-90 ${
                  hasActiveFilters 
                    ? 'bg-red-600 text-white shadow-md shadow-red-600/30' 
                    : 'text-slate-400 hover:text-white hover:bg-white/[0.08]'
                }`}
                title="Advanced Search Filters (Date, Duration, Sort)"
              >
            ...
            {onOpenQuickSettings && (
              <button
                ...
                className="p-1.5 rounded-xl text-slate-400 hover:text-white hover:bg-white/[0.08] active:scale-90 transition-all hidden sm:block"
        ...
        {showSearchDropdown && (
          <div className="absolute top-12 left-0 right-0 bg-slate-950/85 border border-white/[0.12] rounded-3xl shadow-2xl p-2.5 z-50 text-xs text-slate-200 backdrop-blur-2xl animate-in fade-in duration-150">
            {/* Quick Next Episode Smart Shortcut */}
            {(() => {
              const queryEpMatch = searchQuery.match(/(?:ep|episode|ep\.|ep#|part|pt|#)?\s*([0-9]{1,5})/i);
              const epNum = queryEpMatch ? parseInt(queryEpMatch[1], 10) : null;
              if (epNum && epNum > 0 && epNum !== 2024 && epNum !== 2025 && epNum !== 2026 && epNum !== 1080 && epNum !== 720) {
                const prefix = searchQuery.replace(queryEpMatch[0], '').replace(/\s+/g, ' ').trim() || 'Series';
                return (
                  <div className="mb-2 p-2 rounded-2xl bg-gradient-to-r from-red-950/60 to-slate-900 border border-red-500/30 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 text-[11px] text-red-300 font-semibold truncate">
                      <Film size={13} className="text-red-400 flex-shrink-0" />
                      <span className="truncate">Detected Episode: {epNum}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => {
                          const nextQ = `${prefix} episode ${epNum + 1}`.trim();
                          onSearchChange(nextQ);
                          handleSubmit(nextQ);
                        }}
                        className="px-2.5 py-1 rounded-lg bg-red-600 hover:bg-red-500 text-white font-bold text-[10px] flex items-center gap-1 shadow-sm transition-all"
                      >
                        <span>Search Ep {epNum + 1}</span>
                        <FastForward size={10} />
                      </button>
                    </div>
                  </div>
            ...
            <div>
              ...
              {(liveSuggestions.length > 0
                ? liveSuggestions
                : DEFAULT_POPULAR_TOPICS.filter(s => !searchQuery || s.toLowerCase().includes(searchQuery.toLowerCase()))
              ).slice(0, 6).map((sug) => (
                <div
                  key={sug}
                  onClick={() => {
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
        {/* Download FreeTube Android APK Button */}
        {onOpenDownloadApk && (
          <button
            id="header-download-apk-btn"
            onClick={onOpenDownloadApk}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-2xl bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/30 text-emerald-300 hover:text-white transition-all duration-200 active:scale-95 shadow-sm group"
            title="Download FreeTube Android APK"
          >
            <Smartphone size={14} className="text-emerald-400 group-hover:scale-110 transition-transform" />
            <span className="text-xs font-bold hidden sm:inline">APK</span>
          </button>
        )}
        ...
        {/* Profile Avatar Badge matching Screenshot 2 (Orange circle with 'G' for Gangamma) or Incognito */}
        <div 
          id="header-profile-btn"
          onClick={() => onSelectTab('profile')}
          className="flex items-center pl-1 cursor-pointer group relative"
          title={isIncognito ? 'Incognito Mode Active • Profile' : 'You • Profile'}
        >
          {isIncognito ? (
            <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-[12px] font-bold text-white shadow-md ring-2 ring-purple-500/60 border border-white/20">
              🕶️
            </div>
          ) : (
